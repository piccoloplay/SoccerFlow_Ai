...
public class GoalieAgent : Agent
{
    ...
    [SerializeField] private float fieldHalfWidth = 1.6f;

    [Header("─── Spawn palla (relativo arena center) ──────")]
    [Tooltip("Z di partenza della palla (davanti al portiere, lato 'tiratore')")]
    [SerializeField] private float ballSpawnZ      = -1.5f;
    [SerializeField] private float ballSpawnXRange = 2.5f;
    [SerializeField] private float ballSpawnYMin   = 0.3f;
    [SerializeField] private float ballSpawnYMax   = 1.5f;
    [Tooltip("Velocità della palla — costante; con AddRandomness una piccola variazione")]
    [SerializeField] private float ballSpeed       = 8f;
    [Range(0f, 0.5f)]
    [SerializeField] private float ballSpeedJitter = 0.15f;
    ...
    [SerializeField] private float goalZWorld = 5.4f;
    ...
    [SerializeField] private bool  playSaveAnimation     = true;
    [SerializeField] private float saveAnimationDuration = 0.4f;
    [Tooltip("Su gol: piccola pausa per chiarezza visiva prima del reset (off di default per training rapido)")]
    [SerializeField] private bool  pauseOnGoal          = false;
    [SerializeField] private float goalPauseDuration    = 0.2f;

    [Header("─── Debug ─────────────────────────────────────")]
    [Tooltip("Logga le prime N azioni ricevute")]
    [SerializeField] private int debugLogFirstActions = 0;
